...
public class TeleporterFlamonor extends Teleporter
{
    ...
    protected final Long2ObjectMap<TeleporterFlamonor.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap<TeleporterFlamonor.PortalPosition>(4096);

    public TeleporterFlamonor(WorldServer worldIn)
    {
    	super(worldIn);
        this.world = worldIn;
            ...
            for (int j1 = -2; j1 <= 2; ++j1)
            {
                for (int k1 = -2; k1 <= 2; ++k1)
                {
                    for (int l1 = -1; l1 < 3; ++l1)
                    {
                        ...
                        int k2 = k + k1 * 0 - j1 * 1;
                        boolean flag = l1 < 0;
                        this.world.setBlockState(new BlockPos(i2, j2, k2), flag ? RioVBlocks.jaavikBlock.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
        ...
        {
            TeleporterFlamonor.PortalPosition teleporter$portalposition = (TeleporterFlamonor.PortalPosition)this.destinationCoordinateCache.get(l);
            d0 = 0.0D;
            ...
            for (int i1 = -128; i1 <= 128; ++i1)
            {
                ...
                for (int j1 = -128; j1 <= 128; ++j1)
                {
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.world.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2)
                    {
                        ...
                        if (this.world.getBlockState(blockpos1).getBlock() == RioVBlocks.flamonorPortal)
                        {
                            for (blockpos2 = blockpos1.down(); this.world.getBlockState(blockpos2).getBlock() == RioVBlocks.flamonorPortal; blockpos2 = blockpos2.down())
                            {
                                blockpos1 = blockpos2;
                            }
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.put(l, new TeleporterFlamonor.PortalPosition(blockpos, this.world.getTotalWorldTime()));
            }

            double d5 = (double)blockpos.getX() + 0.5D;
            double d7 = (double)blockpos.getZ() + 0.5D;
            BlockPattern.PatternHelper blockpattern$patternhelper = RioVBlocks.flamonorPortal.createPatternHelper(this.world, blockpos);
            ...
            for (int j7 = -1; j7 <= 1; ++j7)
            {
                for (int l7 = 1; l7 < 3; ++l7)
                {
                    for (int k8 = -1; k8 < 3; ++k8)
                    {
                        ...
                        boolean flag = k8 < 0;
                        this.world.setBlockState(new BlockPos(k9, k10, k11), flag ? RioVBlocks.jaavikBlock.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = RioVBlocks.flamonorPortal.getDefaultState().withProperty(BlockPortal.AXIS, l6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
        ...
        for (int i8 = 0; i8 < 4; ++i8)
        {
            for (int l8 = 0; l8 < 4; ++l8)
            {
                for (int l9 = -1; l9 < 4; ++l9)
                {
                    ...
                    this.world.setBlockState(new BlockPos(l10, l11, k12), flag1 ? RioVBlocks.jaavikBlock.getDefaultState() : iblockstate, 2);
                }
            }
    ...
    public void removeStalePortalLocations(long worldTime)
    {
        if (worldTime % 100L == 0L)
        {
            ...
            ObjectIterator<TeleporterFlamonor.PortalPosition> objectiterator = this.destinationCoordinateCache.values().iterator();

            while (objectiterator.hasNext())
            {
                TeleporterFlamonor.PortalPosition teleporter$portalposition = (TeleporterFlamonor.PortalPosition)objectiterator.next();
